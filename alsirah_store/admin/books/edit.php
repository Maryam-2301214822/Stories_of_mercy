...
// جلب بيانات الكتاب وجلب التصنيفات لعرضها
...
if (isset($_POST['update'])) {
    ...
    if (mysqli_query($conn, $query)) {
        ...
        // إعادة جلب البيانات المحدثة لتعرض في الخانات فوراً
        $result = mysqli_query($conn, "SELECT * FROM books WHERE id=$id");
        $book = mysqli_fetch_assoc($result);
    } else {
        $message = "❌ خطأ في تعديل الكتاب: " . mysqli_error($conn);
    }
}
?>
...
<body class="flex min-h-screen">
    ...
    <!-- 2. محتوى الصفحة الرئيسي المطابق لصفحة إضافة كتاب -->
